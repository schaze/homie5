...
//!
//! - the *direct* path (a plain struct field deserialized by serde_yaml_ng itself), where a
...
fn all_values() -> Vec<(&'static str, HomieValue)> {
    vec![
        ...
        (
            "{ Color: 'rgb,255,0,0' }",
            HomieValue::Color(HomieColorValue::RGB(255, 0, 0)),
        ),
        (
            "{ DateTime: '2024-10-08T10:15:30Z' }",
            HomieValue::DateTime(
                chrono::DateTime::parse_from_rfc3339("2024-10-08T10:15:30Z")
                    .unwrap()
                    .into(),
            ),
        ),
...
/// Direct path: the field is deserialized by serde_yaml_ng itself.
...
fn map_form_parses_on_direct_path() {
    for (yaml, expected) in all_values() {
        let parsed: Direct = serde_yaml_ng::from_str(&format!("value: {yaml}"))
            .unwrap_or_else(|e| panic!("direct path failed for `{yaml}`: {e}"));
...
fn map_form_parses_on_buffered_path() {
    for (yaml, expected) in all_values() {
        let parsed: Buffered = serde_yaml_ng::from_str(&format!("value: {yaml}"))
            .unwrap_or_else(|e| panic!("buffered path failed for `{yaml}`: {e}"));
...
fn tag_form_still_parses_on_direct_path() {
    ...
    for (yaml, expected) in cases {
        let parsed: Direct = serde_yaml_ng::from_str(&format!("value: {yaml}"))
            .unwrap_or_else(|e| panic!("tag form failed for `{yaml}`: {e}"));
...
fn empty_parses_as_bare_string_and_map_form() {
    let direct: Direct = serde_yaml_ng::from_str("value: Empty").unwrap();
    assert_eq!(direct.value, HomieValue::Empty);

    let buffered: Buffered = serde_yaml_ng::from_str("value: Empty").unwrap();
    assert_eq!(buffered.into_value(), HomieValue::Empty);

    // JSON-style map form with null payload
    let direct: Direct = serde_yaml_ng::from_str("value: { Empty: null }").unwrap();
    assert_eq!(direct.value, HomieValue::Empty);
}
...
fn json_round_trip_is_unchanged() {
    for (_, value) in all_values() {
        ...
        let back: HomieValue =
            serde_json::from_str(&json).unwrap_or_else(|e| panic!("JSON round trip failed for {json}: {e}"));
    ...
    assert_eq!(
        serde_json::from_str::<HomieValue>("\"Empty\"").unwrap(),
        HomieValue::Empty
    );
}
...
fn unknown_variant_and_multi_key_maps_are_rejected() {
    assert!(serde_yaml_ng::from_str::<Direct>("value: { Boolean: true }").is_err());
    assert!(serde_yaml_ng::from_str::<Direct>("value: { Bool: true, Integer: 5 }").is_err());
    assert!(serde_yaml_ng::from_str::<Direct>("value: NotAVariant").is_err());
}

#[test]
fn invalid_payloads_error_with_context() {
    assert!(serde_yaml_ng::from_str::<Direct>("value: { Duration: nonsense }").is_err());
    assert!(serde_yaml_ng::from_str::<Direct>("value: { DateTime: nonsense }").is_err());
    assert!(serde_yaml_ng::from_str::<Direct>("value: { Color: nonsense }").is_err());
}
